...
for row = 1:H
    ...
    % 补充的 Mpq
    M60 = sum(signalData(row,:).^6) / L;                             % p=6,q=0 -> X^6
    M61 = sum(signalData(row,:).^4 .* abs(signalData(row,:)).^2) / L; % p=6,q=1 -> X^5 X^*
    M62 = sum(signalData(row,:).^2 .* abs(signalData(row,:)).^4) / L; % p=6,q=2 -> X^4 (X^*)^2
    M63 = sum(abs(signalData(row,:)).^6) / L;                         % p=6,q=3 -> |X|^6

    M22 = sum(conj(signalData(row,:)).^2) / L;                        % p=2,q=2 -> (X^*)^2
    M43 = sum(signalData(row,:) .* conj(signalData(row,:)).^3) / L;  % p=4,q=3 -> X (X^*)^3
    ...
    C62 = M62 - 6*M20*M42 - 8*M21*M41 - M22*M40 + 6*M22*M20^2 + 24*M20*M21^2;
    ...
    C60_norm = C60 / (C21_modify^3);
    C61_norm = C61 / (C21_modify^3);
    C62_norm = C62 / (C21_modify^3);
    C63_norm = C63 / (C21_modify^3);
